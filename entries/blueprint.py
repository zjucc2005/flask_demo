# encoding:utf-8
# Blueprints provide a API for encapsulating a group of related routes and templates.
# similar to Rails' controller under namespace
import os
...
def image_upload():
	if request.method == 'POST':
		...
		if form.validate():
			...
			# image file save to path to filename
...
def create():
	if request.method == 'POST':
		...
		if form.validate():
			# set current user to author_id
			entry = form.save_entry(Entry(author = g.user))
			db.session.add(entry)
			db.session.commit()
			# flash(message, category) ,category: ['success','warning','failure','some other words']
